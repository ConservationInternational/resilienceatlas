...
while [ $WAIT_TIME -lt $MAX_WAIT ]; do
    RUNNING=$(docker compose -f "$COMPOSE_FILE" ps --filter "status=running" -q 2>/dev/null | wc -l)
...
if docker compose -f "$COMPOSE_FILE" exec -T backend bundle exec rails db:migrate 2>/dev/null; then
